...
const HistoryItem = ({ name, price, time, verdict, active }) => {
  const verdictColors = {
  ...
  return (
    <aside className="w-64 bg-[#11100e] border-r border-[#262420] flex flex-col h-full z-10 flex-shrink-0">
      ...
      <nav className="p-4 space-y-1 border-b border-[#262420]">
        <a href="#" className="flex items-center gap-3 px-3 py-2 bg-[#1e1c19] text-cyan-400 rounded-md text-sm font-medium">
...
const QuickImportSection = ({ form, setForm }) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2">
          ...
          <div className="flex">
            ...
            />
            <button className="bg-cyan-900/50 hover:bg-cyan-800/50 text-cyan-400 px-4 py-2 rounded-r-md text-sm font-medium border border-l-0 border-[#3a3730] transition-colors">
              Scrape
            </button>
          </div>
        </div>
...
const MainContent = ({ form, setForm, activeMode, setActiveMode, onClearForm, onRunAnalysis }) => (
...
const App = () => {
  ...
  const handleSendChat = () => {
    ...
    setChatInput('');
    setTimeout(() => {
      setChatMessages(prev => [...prev, { sender: 'vera', text: `Processing your request: "${userMsg}". Analysis updated.` }]);
    }, 800);
  };
