...
<aside className="w-64 bg-[#11100e] border-r border-[#262420] flex flex-col h-full z-10 flex-shrink-0">
      ...
      <nav className="p-4 space-y-1 border-b border-[#262420]">
        ...
        <Link href="/analysis" className="flex items-center gap-3 px-3 py-2 text-gray-400 hover:text-gray-200 hover:bg-[#1a1816] rounded-md text-sm font-medium transition-colors">
...
const QuickImportSection = ({ form, setForm, isAnalyzing, onCarfaxResult, onRunAnalysis, photos, setPhotos }: {
  form: any;
  setForm: any;
  isAnalyzing: boolean;
  onCarfaxResult?: (data: any) => void;
  onRunAnalysis: () => void;
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      setForm((f: any) => ({
        ...
        drivetrain: v.drivetrain || f.drivetrain,
        engine: v.engine || f.engine,
  ...
  // photos & setPhotos provided by parent App component (shared state)
...
const ConditionSection = ({ form, setForm, photos, setPhotos }: any) => (
  <section className="bg-[#131210] border border-[#2a2825] rounded-xl overflow-hidden shadow-sm">
    <div className="px-5 py-3 border-b border-[#2a2825] bg-[#161513]">
    ...
    <div className="p-5">
      ...
      <div className="mt-8 pt-8 border-t border-[#2a2825]">
        ...
        <ImageUploader onUpload={(data) => setForm((f: any) => ({ ...f, ...data }))} isLoading={false} initialImages={photos} onImagesChange={setPhotos} />
      </div>
    </div>
  </section>
);
...
const MainContent = ({ form, setForm, activeMode, setActiveMode, onClearForm, onRunAnalysis, isAnalyzing, setIsAnalyzing, photos, setPhotos }: any) => (
  <main className="flex-[3] flex flex-col h-full bg-[#0a0905]">
    ...
    <div className="flex-1 overflow-y-auto p-8" style={customStyles.scrollbarHide}>
      <div className="max-w-4xl mx-auto space-y-8 pb-12">
        <QuickImportSection form={form} setForm={setForm} isAnalyzing={isAnalyzing} onRunAnalysis={onRunAnalysis} photos={photos} setPhotos={setPhotos} />
        ...
        <ConditionSection form={form} setForm={setForm} photos={photos} setPhotos={setPhotos} />
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
        ...
        await fetch('/api/analysis', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ vehicle, result }),
        });
